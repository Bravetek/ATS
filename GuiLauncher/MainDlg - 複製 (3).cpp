    ...
    bool topRight)
{
    ...
    const int padding = 6;
    const int swatch = 12;     // color sample width
    const int gap = 6;      // gap between swatch and text
    const int lineH = 16;     // per-item line height
    ...
    CRect box(x, y, x + boxW, y + boxH);
    mem.FillSolidRect(&box, RGB(255, 255, 255));
    CPen frame; frame.CreatePen(PS_SOLID, 1, RGB(190, 190, 190));
    CPen* ofp = mem.SelectObject(&frame);
    mem.MoveTo(box.left, box.top); mem.LineTo(box.right, box.top);
    mem.LineTo(box.right, box.bottom); mem.LineTo(box.left, box.bottom); mem.LineTo(box.left, box.top);
    mem.SelectObject(ofp);

    int cy = y + padding;
    for (auto& it : items) {
        COLORREF clr = it.second;
        // swatch: small colored line
