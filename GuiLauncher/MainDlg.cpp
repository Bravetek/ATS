...
// forward declarations（讓整個 MainDlg.cpp 都看得到）
static COLORREF HsvToRgb(double h, double s, double v);
static COLORREF ColorFromIndex(size_t idx, double sat = 0.80, double val = 0.95);
static void DrawLegend(CDC& mem, const CRect& plotRect,
    const std::vector<std::pair<CString, COLORREF>>& items,
    bool topRight = true);
...
//        double value
        ...
        if (line.find("\"type\": \"kpi\"") != std::string::npos) { //important space after type :" "//
            ...
            //    m_colors[key] = table[m_colors.size() % _countof(table)];
            //}
            if (m_colors.find(key) == m_colors.end()) { m_colors[key] = ColorFromIndex(m_colors.size()); }
...
// ==== Color helpers (HSV -> RGB, golden-angle palette) ====
static COLORREF HsvToRgb(double h, double s, double v)
{
...
// Consistent vivid palette: step hue by golden ratio for maximal contrast
static COLORREF ColorFromIndex(size_t idx, double sat, double val)
{
    const double GOLDEN = 0.6180339887498949;
    double h = fmod(0.27 + GOLDEN * double(idx), 1.0);
    ...
    bool topRight)
{
    ...
    // Font for measuring / drawing
    CFont f; f.CreatePointFont(80, L"Segoe UI"); // 8pt
    ...
    int textW = 0;
    for (auto& it : items) {
        CSize sz = mem.GetTextExtent(it.first);
        textW = max(textW, sz.cx);
    }
    int boxW = padding + swatch + gap + textW + padding;
    int boxH = padding + (int)items.size() * lineH + padding;

    int x = topRight ? (plotRect.right - boxW - 4) : (plotRect.left + 4);
    int y = plotRect.top + 4;

    // ---- Pass 1: semi-transparent white background using AlphaBlend ----
    CDC memBg; memBg.CreateCompatibleDC(&mem);
    CBitmap bmp; bmp.CreateCompatibleBitmap(&mem, boxW, boxH);
    HGDIOBJ oldBmp = memBg.SelectObject(&bmp);
    CRect bgRc(0, 0, boxW, boxH);
    memBg.FillSolidRect(&bgRc, RGB(255, 255, 255));     // white panel

    BLENDFUNCTION bf = { AC_SRC_OVER, 0, 125 /* ~63% opacity */, 0 /* constant alpha */ };
    ::AlphaBlend(mem.GetSafeHdc(), x, y, boxW, boxH,
        memBg.GetSafeHdc(), 0, 0, boxW, boxH, bf);
    memBg.SelectObject(oldBmp);

    // ---- Pass 2: opaque border + content (keeps text/readability) ----
    // border
    CPen frame; frame.CreatePen(PS_SOLID, 1, RGB(190, 190, 190));
    CPen* ofp = mem.SelectObject(&frame);
    mem.MoveTo(x, y); mem.LineTo(x + boxW, y);
    mem.LineTo(x + boxW, y + boxH); mem.LineTo(x, y + boxH); mem.LineTo(x, y);
    mem.SelectObject(ofp);

    int cy = y + padding;
    for (auto& it : items) {
        COLORREF clr = it.second;
        // swatch line
        CPen pen; pen.CreatePen(PS_SOLID, 2, clr);
        CPen* op = mem.SelectObject(&pen);
        int sy = cy + lineH / 2;
        mem.MoveTo(x + padding, sy);
        mem.LineTo(x + padding + swatch, sy);
        mem.SelectObject(op);

        // text
        CRect tr(x + padding + swatch + gap, cy, x + boxW - padding, cy + lineH);
        mem.DrawTextW(it.first, tr, DT_LEFT | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
        cy += lineH;
    }
...
void CMainDlg::DrawChart() {
    ...
    // ---- legend (series=(i1,i23)) ----
    std::vector<std::pair<CString, COLORREF>> legendItemsL;
    legendItemsL.reserve(m_series.size());
    for (auto& kv : m_series) {
        CString label;
        label.Format(L"I1=%.3fA, I23=%.3fA", kv.first.i1, kv.first.i23);
        COLORREF clr = RGB(0, 0, 0);
        auto itc = m_colors.find(kv.first);
        if (itc != m_colors.end()) clr = itc->second;
        legendItemsL.emplace_back(label, clr);
    }
    DrawLegend(mem, rc, legendItemsL, true);
    dc.BitBlt(0, 0, rc.Width(), rc.Height(), &mem, 0, 0, SRCCOPY); mem.SelectObject(oldBmp);
    ...
    double xmin = 1e9, xmax = -1e9, ymin = 1e9, ymax = -1e9;
    for (auto& kv : m_series) for (auto& pt : kv.second) {
        double vx = (pt.x > 20.0 ? pt.x / 100.0 : pt.x); // auto-handle centivolts
        double vy = pt.y;
        if (vy > 5.0 && vy <= 150.0) vy /= 100.0;   // percent -> 0..1
        xmin = min(xmin, vx); xmax = max(xmax, vx);
        ymin = min(ymin, vy); ymax = max(ymax, vy);
    }
    ...
    //auto X = [&](double x) { return rc.left + margin + int((x - xmin) / (xmax - xmin + 1e-12) * w); };
    //auto Y = [&](double y) { return rc.bottom - margin - int((y - ymin) / (ymax - ymin + 1e-12) * h); };
        // plot rect with margins for ticks/labels
    const int marginL = 52, marginR = 8, marginT = 8, marginB = 28;
    CRect prc = rc; prc.DeflateRect(marginL, marginT, marginR, marginB);

    // ticks
    double x0, xstep; int nx; CalcTicks(xmin, xmax, 6, x0, xstep, nx);
    double y0, ystep; int ny; CalcTicks(ymin, ymax, 6, y0, ystep, ny);

    auto X = [&](double x) { return prc.left + int((x - x0) / (xstep * (nx - 1) + 1e-12) * prc.Width()); };
    auto Y = [&](double y) { return prc.bottom - int((y - y0) / (ystep * (ny - 1) + 1e-12) * prc.Height()); };

    // axes lines
    CPen axis; axis.CreatePen(PS_SOLID, 1, RGB(160, 160, 160));
    CPen grid; grid.CreatePen(PS_DOT, 1, RGB(220, 220, 220));
    CPen* oPen = mem.SelectObject(&axis);
    mem.MoveTo(prc.left, prc.bottom); mem.LineTo(prc.right, prc.bottom); // X
    mem.MoveTo(prc.left, prc.bottom); mem.LineTo(prc.left, prc.top);    // Y
    mem.SelectObject(oPen);

    // tick labels font
    CFont fTick; fTick.CreatePointFont(80, L"Segoe UI"); // 8pt
    CFont* of = mem.SelectObject(&fTick);

    // X ticks
    for (int i = 0; i < nx; i++) {
        double xv = x0 + i * xstep;
        int px = X(xv), py = prc.bottom;
        mem.MoveTo(px, py); mem.LineTo(px, py + 4);
        CString s = FormatTick(xv);
        CRect tr(px - 30, py + 4, px + 30, py + 22);
        mem.DrawTextW(s, tr, DT_CENTER | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
        // gridline
        CPen* og = mem.SelectObject(&grid); mem.MoveTo(px, prc.top); mem.LineTo(px, prc.bottom); mem.SelectObject(og);
    }
    // Y ticks
    for (int j = 0; j < ny; j++) {
        double yv = y0 + j * ystep;
        int py = Y(yv), px = prc.left;
        mem.MoveTo(px - 4, py); mem.LineTo(px, py);
        CString s = FormatTick(yv);
        CRect tr(px - 48, py - 8, px - 6, py + 8);
        mem.DrawTextW(s, tr, DT_RIGHT | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
        // gridline
        CPen* og = mem.SelectObject(&grid); mem.MoveTo(prc.left, py); mem.LineTo(prc.right, py); mem.SelectObject(og);
    }

    // axis labels (Vin, Efficiency)
    CFont fLbl; fLbl.CreatePointFont(90, L"Segoe UI Semibold");
    mem.SelectObject(&fLbl);
    CRect tx(prc.left, rc.bottom - 24, prc.right, rc.bottom);
    mem.DrawTextW(L"Vin (V)", tx, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    mem.TextOutW(rc.left + 4, prc.top, L"Efficiency");
    ...
    for (auto& kv : m_series) {
        ...
        CPen pen(PS_SOLID, 2, clr); CPen* old = mem.SelectObject(&pen);
        ...
        for (auto& pt : kv.second) {
            double vx = (pt.x > 20.0 ? pt.x / 100.0 : pt.x);
            double vy = pt.y; if (vy > 5.0 && vy <= 150.0) vy /= 100.0;
            pts.emplace_back(vx, vy);
        }
        std::sort(pts.begin(), pts.end(), [](auto& a, auto& b) {return a.first < b.first; });

        for (auto& p : pts) {
            int x = X(p.first), y = Y(p.second);
            if (!first) mem.LineTo(x, y); else mem.MoveTo(x, y);
...
{
    // Right plot: X = i23, Y = efficiency; series = Vin
    ...
    std::map<double, std::vector<XY>> byVin;
    std::map<double, COLORREF> colorVin;
    double xmin = 1e9, xmax = -1e9, ymin = 1e9, ymax = -1e9;


    for (auto& kv : m_series) {
        double i23 = kv.first.i23;
        for (auto& pt : kv.second) {
            double vin = (pt.x > 20.0 ? pt.x / 100.0 : pt.x);
            double eta = pt.y; if (eta > 5.0 && eta <= 150.0) eta /= 100.0;
            if (eta < 0.0 || eta>1.5) continue;
            byVin[vin].push_back({ i23, eta });
            xmin = min(xmin, i23); xmax = max(xmax, i23);
            ymin = min(ymin, eta); ymax = max(ymax, eta);
            if (colorVin.find(vin) == colorVin.end()) { colorVin[vin] = ColorFromIndex(colorVin.size()); }
        }
    }
    if (!(ymax > ymin)) { ymin = 0.0; ymax = 1.0; }
    ...
    CPen grid; grid.CreatePen(PS_DOT, 1, RGB(220, 220, 220));
    CPen* oPen = mem.SelectObject(&axis);
    mem.MoveTo(prc.left, prc.bottom); mem.LineTo(prc.right, prc.bottom);
    mem.MoveTo(prc.left, prc.bottom); mem.LineTo(prc.left, prc.top);
    mem.SelectObject(oPen);

    CFont fTick; fTick.CreatePointFont(80, L"Segoe UI"); CFont* of = mem.SelectObject(&fTick);
    for (int i = 0; i < nx; i++) {
        double xv = x0 + i * xstep; int px = X(xv), py = prc.bottom;
        mem.MoveTo(px, py); mem.LineTo(px, py + 4);
        CString s = FormatTick(xv); CRect tr(px - 30, py + 4, px + 30, py + 22);
        mem.DrawTextW(s, tr, DT_CENTER | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
        CPen* og = mem.SelectObject(&grid); mem.MoveTo(px, prc.top); mem.LineTo(px, prc.bottom); mem.SelectObject(og);
    }
    for (int j = 0; j < ny; j++) {
        double yv = y0 + j * ystep; int py = Y(yv), px = prc.left;
        mem.MoveTo(px - 4, py); mem.LineTo(px, py);
        CString s = FormatTick(yv); CRect tr(px - 48, py - 8, px - 6, py + 8);
        mem.DrawTextW(s, tr, DT_RIGHT | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
        CPen* og = mem.SelectObject(&grid); mem.MoveTo(prc.left, py); mem.LineTo(prc.right, py); mem.SelectObject(og);
    }

    CFont fLbl; fLbl.CreatePointFont(90, L"Segoe UI Semibold"); mem.SelectObject(&fLbl);
    CRect tx(prc.left, rc.bottom - 24, prc.right, rc.bottom);
    mem.DrawTextW(L"i23 (A)", tx, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    mem.TextOutW(rc.left + 4, prc.top, L"Efficiency");

    for (auto& kv : byVin) {
        COLORREF clr = colorVin[kv.first];
        CPen pen; pen.CreatePen(PS_SOLID, 2, clr);
        CPen* old = mem.SelectObject(&pen);
        auto pts = kv.second;
        std::sort(pts.begin(), pts.end(), [](const XY& a, const XY& b) { return a.x < b.x; });
        bool first = true; for (auto& p : pts) {
            int x = X(p.x), y = Y(p.y);
            if (!first) mem.LineTo(x, y); else mem.MoveTo(x, y);
            first = false; mem.Ellipse(x - 2, y - 2, x + 2, y + 2);
        }
        mem.SelectObject(old);
    }

    // legend (now colorVin is ready)
    std::vector<std::pair<CString, COLORREF>> legendItemsR;
    std::vector<double> vinKeys; vinKeys.reserve(colorVin.size());
    for (auto& p : colorVin) vinKeys.push_back(p.first);
    std::sort(vinKeys.begin(), vinKeys.end());
    for (double v : vinKeys) {
        CString label; label.Format(L"Vin=%.2f V", v);
        legendItemsR.emplace_back(label, colorVin[v]);
    }
    DrawLegend(mem, prc, legendItemsR, true);
